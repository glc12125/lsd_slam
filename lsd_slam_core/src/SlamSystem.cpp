    ...
    else
    {
        if(force)
        {
            if(noCreate)
            {
                ...
                std::cout << "[changeKeyframe] createNewCurrentKeyframe\n";
...
bool SlamSystem::updateKeyframe()
{
    ...
    {
        std::cout << "[updateKeyframe] will call map->updateKeyframe()\n";
...
bool SlamSystem::doMappingIteration()
{
    ...
    {   
        ...
        {
            std::cout << "[doMappingIteration] calling finishCurrentKeyframe\n";
            finishCurrentKeyframe();
...
void SlamSystem::trackFrame(uchar* image, unsigned int frameID, bool blockUntilMapped, double timestamp)
{
    ...
    {
        std::cout << "[trackFrame] importFrame from currentKeyFrame\n";
        trackingReference->importFrame(currentKeyFrame.get());
        currentKeyFrame->depthHasBeenUpdatedFlag = false;
        trackingReferenceFrameSharedPT = currentKeyFrame;
    }
    ...
    if (!my_createNewKeyframe && currentKeyFrame->numMappedOnThisTotal > MIN_NUM_MAPPED)
    {
        ...
        float minVal = fmin(keyFrameGraph->keyframesAll.size() * 0.1f / INITIALIZATION_PHASE_COUNT,1.0f);
        ...
        std::cout << "[trackFrame] minVal: " << minVal << "\n";
